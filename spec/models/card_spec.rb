  ...
  describe "#strength_in_row(row)" do
    ...
    context "when weather effects are active" do
      ...
      it "leaves cards with 0 strength unchanged" do
        subject.strength = 0
        expect( subject.strength_in_row( row ) ).to eq 0
      end
      it "reduces Unit strength to 1" do
        subject.update(card_type: 'Unit', strength: 2)
        expect( subject.strength_in_row(row) ).to eq 1
      end
      it "leaves Hero strength unaffected" do
        expect( hero.strength_in_row(row) ).to eq hero.strength
      end
    end
    ...
    context "when Morale Boost ability is present" do
      it "adds boost count to each unit" do
        ...
        expect( subject.strength_in_row(row) ).to eq( subject.strength + 1 )
      end
      it "leaves Hero strength unaffected" do
        expect( hero.strength_in_row(row) ).to eq hero.strength
      end
    ...
    context "when Tight Bond ability is present" do
      it "doubles each related card by N times, where N is the number of related bond cards" do
        allow(row).to receive(:tight_bonds_for).with(subject).and_return(1)
        expect( subject.strength_in_row(row) ).to eq subject.strength * 2
        allow(row).to receive(:tight_bonds_for).with(subject).and_return(2)
        expect( subject.strength_in_row(row) ).to eq subject.strength * 4
      end
    end
    ...
    context "when commander's horn is active" do
      ...
      it "doubles Unit strength" do
        subject.update(card_type: 'Unit', strength: 2)
        expect( subject.strength_in_row(row) ).to eq 4
      end
      it "leaves Hero strength unaffected" do
        expect( hero.strength_in_row(row) ).to eq hero.strength
      end
